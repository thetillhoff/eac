...
func downloadApp(conf config.Config, app app.App, platform string) {
	...
	downloadFile("https://raw.githubusercontent.com/thetillhoff/eac/main/apps/"+app.Name+"/"+platform+"/getInstalledVersion.sh", path.Join(conf.AppsDirPath, app.Name, platform, "getInstalledVersion.sh"))
