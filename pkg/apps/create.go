...
// Creates the default files and folder structure for the provided list of appnames
func Create(appNames []string, flaggedPlatforms []string, appsDirPath string, verbose bool, createData map[string]string) {
	...
	if _, err := os.Stat(appsDirPath); os.IsNotExist(err) {
		logs.Error("Apps folder at '" + appsDirPath + "' doesn't exist.\nPlease run 'eac init' first.")
	} else if err != nil {
		logs.Error("There was an error while accessing the appsDir at '"+appsDirPath+"':", err)
	}
	...
	for _, appName := range appNames {
		...
		if _, err := os.Stat(appPath); os.IsNotExist(err) { // if folder doesn't exist yet
			...
			if err != nil {
				logs.Error("There was an error while creating the app at '"+appPath+"':", err)
			}
			...
			logs.Error("There was an error while accessing the app at '"+appPath+"':", err)
		}
		...
		for _, platform := range platforms {
			...
			if _, err := os.Stat(platformPath); os.IsNotExist(err) {
				...
				if err != nil {
					logs.Error("There was an error while creating the platform '"+platform+"' for app '"+appPath+"' at '"+platformPath+"':", err)
				}
				logs.Info("Created '" + platformPath + "' folder.")
			} else if err == nil {
				logs.Warn("Platform '" + platform + "' for app '" + appName + "' does already exist.")
			} else {
				logs.Error("There was an error while accessing the platform '"+platform+"' for app '"+appPath+"' at '"+platformPath+"':", err)
			}
			...
			if githubUser, ok := createData["githubUser"]; ok { // add github files
				...
				case "windows":
					...
					templates.WriteTemplateToFile(path.Join(platform, "default-getInstalledVersion.ps1"), data, path.Join(platformPath, "getInstalledVersion.ps1"))
				...
				default:
					...
					templates.WriteTemplateToFile(path.Join(platform, "default-getInstalledVersion.sh"), data, path.Join(platformPath, "getInstalledVersion.sh"))
