...
wget -q "https://github.com/{{ index . "user" }}/{{ index . "name" }}/releases/download/v$1/{{ index . "name" }}-linux-amd64" -O "$2/{{ index . "name" }}-linux-amd64"
sudo install -D -g root -o root -m 755 "$2/{{ index . "name" }}-linux-amd64" "/usr/local/bin/{{ index . "name" }}"
...
sudo ~/.eac/shared/addToGlobalPath.sh "/usr/local/bin/{{ index . "name" }}"
